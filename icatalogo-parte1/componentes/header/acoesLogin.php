  ...
  switch(isset($_POST["acao"])){
    case "login":
        if(isset($_POST['usuario']) && isset($_POST['senha'])){
          ...
          $verificaUsuario = mysqli_query($conexao, "SELECT usuario FROM tbl_administrador WHERE usuario = '$usuario';");
          $verificaSenha = mysqli_query($conexao, "SELECT senha FROM tbl_administrador WHERE senha = '$senha';");
          if(mysqli_num_rows($verificaUsuario) == 1 && mysqli_num_rows($verificaSenha) == 1){
